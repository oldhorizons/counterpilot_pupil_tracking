...
debug = False #generates lists of options, ignores given parameters
verbose = False #shows everything generated
...
class Tracker:
        ...
        match pupilModel:
            ...
            case _:
                ...
                if self.pupilModelParams["post"] == "blob":
                    ...
                    params.filterByIntertia = True #shape longness
    ...
    def find_face(self, cv2Image):
        ...
        if debug:
            if verbose:
                rects = self.draw_all_rectangles(cv2Image, faces, maxFace, "Faces Detected")
    ...
    def process_opencv(self, cv2Image, method, show=False): #TODO YOU ARE CURRENTLY DRAWING THESE TWICE YOU *GOOSE*
        """
    ...
    def get_processed_list(self, cv2Image, preStages = "processed"):
        ...
        #TODO figure out how to return the list!!

    #tester function for 'quick and dirty' / data-based pupil detection TODO FINISH
    def find_pupil_cv2(self, cv2Image):
        if debug:
            pre, preLabels = self.get_preprocessed_list(cv2Image)
            post, postLabels = self.get_opencv_pupil_list(pre, preLabels)
        else:
            for step in self.pupilModelParams['pre']:
                cv2Image = self.preprocess_opencv(cv2Image, step)
            self.process_opencv(cv2Image, self.pupilModelParams['post'])
